#!/usr/bin/env bash
# Analyze Galaxea lerobot_press (R1 Lite): pointer → percentiles → Stage 1/2/3 → threshold report
# Outputs under tests_au/ops/filter/outputs/press_analyze/ (does not overwrite full_analyze/).
set -euo pipefail
...
OUT="$SCRIPT_DIR/outputs/press_analyze"
...
echo "=== lerobot_press analyze (embodiment=$EMBODIMENT) ==="
echo "ROOT=$ROOT"
echo "OUT=$OUT"
...
if ! "$VENV/bin/dj-analyze" --config "$SCRIPT_DIR/analyze_lerobot_press.yaml"; then
...
out = Path("tests_au/ops/filter/outputs/press_analyze")
