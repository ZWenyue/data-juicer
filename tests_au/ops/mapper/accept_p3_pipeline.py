...
Outputs JSON Go/No-Go under --output-dir.
"""
...
def _make_synthetic_sample(tmp: Path, n_frames: int = 8, side: str = "right"):
    ...
    for i in range(n_frames):
        ...
        frames.append(str(p))
        d = np.full((240, 320), 2.5, dtype=np.float32)
    ...
    return {
        ...
        Fields.meta: {
            ...
            MetaKeys.hand_reconstruction_hawor_tags: [
                {side: {"frame_ids": list(range(n_frames)), "joints_cam": joints}}
            ],
            ...
            MetaKeys.camera_calibration_moge_tags: [
                {
                    CameraCalibrationKeys.depth: depths,
                    CameraCalibrationKeys.intrinsics: [K] * n_frames,
                }
            ],
...
def main():
    ...
    try:
        ...
        caption_op = VideoHandActionCaptionStubMapper(
            hand_type=args.side,
            frame_field="robot_render_frames",
        )
        sample = caption_op.process_single(sample)
        ...
        actions_after = snapshot_hand_actions(sample)
        inv_ok, inv_report = compare_action_invariance(actions_before, actions_after)
        ...
        report["checks"]["video_encoded_from_frames"] = any(
            ep.get("encoded_from_frames") for ep in export_meta
        )
        ...
        report["checks"]["robot_outputs_differ"] = all(
            a != b for a, b in zip(out_frames, src_frames) if a and b
        )

        ik_rate = report["render_quality"].get("ik_success_rate", 0.0)
        # Synthetic clip uses bootstrap v1 calib; pipeline wiring is primary gate.
        report["checks"]["ik_success_ge_0_5"] = float(ik_rate) >= 0.5
        ...
        go = all(
            ...
            report["checks"].get("robot_outputs_differ") or report["checks"].get("ik_success_ge_0_5")
        )
