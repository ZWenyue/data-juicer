# Galaxea Open-World Dataset 分析: Connect_Router_Cables 任务 (Data-Juicer 版)

> **TL;DR** — 对 Galaxea R1 Lite 机器人的 Connect_Router_Cables 任务（125 episodes, 209K frames, 4 cameras × 720p@15fps）进行了多层次数据质量分析。**所有分析和清洗均通过 Data-Juicer 自定义算子实现**（`custom_ops_2.py`），无外部脚本依赖。发现三个关键问题：① Episode 0 混入了冰箱操作等无关任务（应移除）；② **44.9% 的帧被标记为 unqualified**（质量标签为帧级分段标注）；③ 32/125 episodes 存在 state-action 方向不一致。轨迹平滑度（SAL）中位数 −420，最粗糙的 Episode 0 为 −2813。建议通过 DJ 管道进行分层过滤和信号平滑。
...
**DJ 实现**: `trajectory_sal_filter` (`custom_ops_2.py`) 在 `compute_stats_single` 中对每个 episode 的左右臂关节速度计算 SAL，存入 `Fields.stats`。
...
**DA < 0.65 表示该维度的 state 与 action 趋势不一致**，可能由时钟不同步、丢包、或控制延迟导致。该阈值参考了 RINSE 论文中对数据一致性的分析。

**DJ 实现**: `trajectory_da_filter` (`custom_ops_2.py`) 对 12 个手臂关节维度计算 DA，记录不对齐维度数。
...
**DJ 实现**: `trajectory_sudden_change_filter` (`custom_ops_2.py`) 在 `compute_stats_single` 中实现。

---

## 4. Connect_Router_Cables 质量分析结果

> 以下结果由 `analyze_glx_2.py` 通过 Data-Juicer 自定义算子计算得出。
...
**DJ 检测**: `episode_off_task_filter` 通过关键词匹配（"冰箱", "refrigerator", "蘑菇" 等）自动标记。
...
**DJ 检测**: `quality_label_filter` 读取每个 episode parquet 的 `quality_index` 列，统计 qualified/unqualified 帧数。
...
> **解释**: 这可能是因为 `lerobot_door` 版本已经经过了预处理（统一到 unified 格式时可能做了初步平滑），或者原始数据本身的采集质量较好，不存在严重的碰撞/丢包毛刺。阈值 $8\sigma$ 是比较保守的设定，可以在 DJ 配置中将 `c_threshold` 降到 5.0 来捕获更轻微的抖动。
...
| 优先级 | 问题 | DJ 算子 | 预期效果 |
|--------|------|---------|---------|
| **P0** | Episode 0 混入无关任务 | `episode_off_task_filter` | 消除噪声数据 |
| **P0** | 44.9% 帧标记为 unqualified | `quality_label_filter` (Stage 3 设 `min_qualified_ratio: 0.5`) | 保证 few-shot 数据质量 |
| **P1** | 32 episodes state-action 不对齐 | `trajectory_da_filter` | 移除不可靠轨迹 |
| **P1** | SAL < −1000 的粗糙轨迹 | `trajectory_sal_filter` (设 `min_sal: -1000`) | 提升训练收敛速度 |
| **P2** | 关节运动毛刺 | Savitzky-Golay 平滑（DJ `video_hand_motion_smooth_mapper` 同款算法） | 降低动作噪声 |
| **P2** | 夹爪传感器噪声 | 可扩展 `trajectory_stats_mapper` 添加 clip 逻辑 | 改善抓取/释放检测 |

### 5.2 DJ 管道清洗方案

所有清洗步骤均通过 Data-Juicer 算子实现，可通过 YAML 配置灵活调整阈值：

#### P0: Episode 过滤

```yaml
# dj_analyze_2.yaml 中的配置
- episode_off_task_filter:
    off_task_keywords: ['冰箱', 'refrigerator', '蘑菇', 'mushroom']
```

DJ 管道自动检测并移除包含无关关键词的 episode。

#### P0: 基于质量标签的帧级过滤

```yaml
- quality_label_filter:
    qualified_task_index: 25
    unqualified_task_index: 9
    min_qualified_ratio: 0.5  # Stage 3 微调时设置
```

#### P1: DA 方向一致性过滤

```yaml
- trajectory_da_filter:
    da_thresh: 0.65
    max_misaligned_dims: 0  # 严格模式：任何维度不对齐都过滤
```

#### P1: SAL 平滑度排序筛选

```yaml
- trajectory_sal_filter:
    min_sal: -1000.0  # 过滤最粗糙的 5%
```

#### P2: 运动平滑化

Data-Juicer 的 `video_hand_motion_smooth_mapper` 使用以下流程（其算法可复用于关节数据）：
...
可通过扩展 `custom_ops_2.py` 中添加 `TrajectorySmoothmapper` 来实现。

### 5.3 清洗后预期数据量

```mermaid
flowchart LR
    A["125 episodes<br/>209K frames"] --> B["P0: episode_off_task_filter<br/>-1 episode (-8,278 frames)"]
    B --> C["P1: trajectory_da_filter<br/>-32 episodes"]
    C --> D["P1: trajectory_sal_filter<br/>-5 episodes (SAL < -1000)"]
    D --> E["清洗后: ~87 episodes<br/>~140K frames<br/>Stage 2 可用"]

    F["同上 ~87 episodes"] --> G["P0: quality_label_filter<br/>仅 qualified 帧 (×55.1%)"]
    G --> H["~77K qualified frames<br/>Stage 3 可用"]
...
所有轨迹质量分析功能通过 DJ 自定义算子实现，注册到 DJ 的 `OPERATORS` 全局注册表中，可被 YAML 配置直接引用：

```mermaid
flowchart TB
    subgraph OPS["custom_ops_2.py — DJ 自定义算子"]
        direction TB
        F1["episode_off_task_filter<br/>→ Filter: 关键词匹配"]
        F2["trajectory_sal_filter<br/>→ Filter: FFT 频域分析"]
        F3["trajectory_da_filter<br/>→ Filter: 差分方向比较"]
        F4["trajectory_sudden_change_filter<br/>→ Filter: 加速度阈值"]
        F5["quality_label_filter<br/>→ Filter: parquet quality_index"]
        M1["trajectory_stats_mapper<br/>→ Mapper: 关节/夹爪统计"]
    end

    subgraph DJ["Data-Juicer 框架"]
        REG["OPERATORS.register_module()"]
        API["NestedDataset.process()"]
        CFG["YAML 配置 / dj-process"]
    end

    OPS --> REG
    REG --> API & CFG

    style OPS fill:#e8f4fd,stroke:#4a90d9
    style DJ fill:#f4fff0,stroke:#66aa66
```

每个 Filter 算子继承 DJ 的 `Filter` 基类，实现两个方法：
- `compute_stats_single(sample)`: 从 `parquet_path` 读取数据，计算指标，存入 `sample[Fields.stats]`
- `process_single(sample)`: 根据 stats 中的指标和阈值返回 `True`（保留）或 `False`（过滤）

### 6.2 DJ 算子适配性评估

| 需求 | 实现方式 | 适配级别 | 说明 |
|------|---------|---------|------|
| Episode 元数据过滤 | `episode_off_task_filter` (自定义 Filter) | 🟢 | 关键词匹配，已实现 |
| SAL 计算与排序 | `trajectory_sal_filter` (自定义 Filter) | 🟢 | FFT 频域分析，已实现 |
| State-action 对齐检测 | `trajectory_da_filter` (自定义 Filter) | 🟢 | 差分方向比较，已实现 |
| 突变检测 | `trajectory_sudden_change_filter` (自定义 Filter) | 🟢 | 加速度阈值，已实现 |
| 帧级质量标签过滤 | `quality_label_filter` (自定义 Filter) | 🟢 | quality_index 统计，已实现 |
| 轨迹统计 | `trajectory_stats_mapper` (自定义 Mapper) | 🟢 | 关节范围/夹爪分析，已实现 |
| 运动平滑化 | DJ 内置 `video_hand_motion_smooth_mapper` 算法可复用 | 🔵 | 需适配关节空间数据 |
| 子任务分割 | DJ 内置 `video_atomic_action_segment_mapper` | 🔵 | 速度极值分割，可适配关节速度 |
| 视频帧提取 | DJ 内置 `video_extract_frames_mapper` | 🟢 | 直接可用 |
| 深度估计 | DJ 内置 `video_depth_estimation_mapper` | 🟢 | 可补充深度信息（原数据集无） |
| 文本标注质量 | DJ 内置 `text_length_filter`, `language_id_score_filter` | 🟢 | 通过 `convert_to_jsonl_2.py` 转换后可用 |
| LeRobot 格式导出 | DJ 内置 `export_to_lerobot_mapper` | 🟢 | 清洗后可重新导出 |

> **图标说明**: 🟢 已实现/直接可用 · 🔵 组合/适配可用
...
flowchart TB
    ...
    subgraph Convert["数据转换"]
        C1["convert_to_jsonl_2.py<br/>→ episodes_2.jsonl<br/>(per-episode DJ 格式)"]
    end

    subgraph Stage_A["Stage A: Episode 过滤"]
        F1["① episode_off_task_filter<br/>(移除无关任务 episode)"]
        F5["② quality_label_filter<br/>(统计 qualified 帧比例)"]
    end

    subgraph Stage_B["Stage B: 轨迹质量过滤"]
        F2["③ trajectory_sal_filter<br/>(SAL 平滑度阈值)"]
        F3["④ trajectory_da_filter<br/>(state-action 对齐)"]
        F4["⑤ trajectory_sudden_change_filter<br/>(加速度突变)"]
    end

    subgraph Stage_C["Stage C: 数据增强 (可选)"]
        E1["⑥ video_extract_frames_mapper<br/>(DJ 内置)"]
        E2["⑦ video_depth_estimation_mapper<br/>(DJ 内置)"]
        E3["⑧ video_atomic_action_segment_mapper<br/>(DJ 内置)"]
    end
    ...
    style Convert fill:#e0f0ff,stroke:#4488cc
    style Stage_A fill:#ffe8e8,stroke:#cc4444
    style Stage_B fill:#fff3e0,stroke:#cc8844
    style Stage_C fill:#e8f5e8,stroke:#44aa44
...
#### 方式二：Python API（推荐，可生成完整报告）

```bash
/mnt/r/VENV/dj/bin/python b/dm/glxOpnWld/analyze_glx_2.py
```

Python API 的优势：可在运行算子后收集 `Fields.stats`，聚合为结构化 JSON 报告。
...
```python
# 1. 继承 Filter 或 Mapper 基类
from data_juicer.ops.base_op import OPERATORS, Filter

# 2. 注册到 DJ
@OPERATORS.register_module("my_new_filter")
class MyNewFilter(Filter):

    def __init__(self, threshold=0.5, parquet_field="parquet_path",
                 *args, **kwargs):
    ...
    def compute_stats_single(self, sample, context=False):
        states, actions, df = _load_episode_arrays(
            sample[self.parquet_field])
        metric = compute_my_metric(states, actions)
        sample[Fields.stats]["my_metric"] = metric
        return sample

    # 3b. 过滤决策（处理模式会调用）
    def process_single(self, sample):
        return sample[Fields.stats]["my_metric"] >= self.threshold
```
...
| 文件 | 用途 |
|------|------|
| `analysis_2.md` | 本文档 — 完整分析报告 |
| `custom_ops_2.py` | DJ 自定义算子（SAL、DA、突变检测、质量标签、统计） |
| `analyze_glx_2.py` | DJ Python API 分析脚本 |
| `analysis_results_2.json` | 分析脚本输出的完整 JSON 报告 |
| `dj_analyze_2.yaml` | Data-Juicer YAML 配置（分析/处理模式） |
| `convert_to_jsonl_2.py` | 将 LeRobot 数据转为 DJ per-episode JSONL |
| `episodes_2.jsonl` | 转换后的 per-episode 数据 |

### 7.2 运行命令速查

```bash
# 1. 转换 LeRobot 数据为 DJ 格式
/mnt/r/VENV/dj/bin/python b/dm/glxOpnWld/convert_to_jsonl_2.py

# 2. 运行完整质量分析（DJ Python API，生成 JSON 报告）
/mnt/r/VENV/dj/bin/python b/dm/glxOpnWld/analyze_glx_2.py

# 3. 运行 DJ 分析器（统计模式）
/mnt/r/VENV/dj/bin/dj-analyze \
  --config b/dm/glxOpnWld/dj_analyze_2.yaml

# 4. 运行 DJ 处理器（过滤模式，实际清洗数据）
/mnt/r/VENV/dj/bin/dj-process \
  --config b/dm/glxOpnWld/dj_analyze_2.yaml
```
...
`trajectory_sal_filter` 在 `compute_stats_single` 中调用 `_spectral_arc_length()` 函数：
...
def _spectral_arc_length(speed_signal, dt=DT, eps=1e-5):
    ...
    return float(-np.sum(np.sqrt(np.diff(freqs)**2 + np.diff(L)**2)))
```

对每个 episode 分别计算左右臂的 end-effector SAL（6 关节速度的范数）和逐关节 SAL 均值。
...
**DA = 1.0** 意味着 state 和 action 在每个有效运动帧都朝同一方向变化——完美一致。**DA = 0.5** 意味着一致性和随机乱猜一样——严重问题。阈值设为 **0.65**，参考 RINSE 论文中对数据一致性的分析。

#### DJ 实现

`trajectory_da_filter` 的 `compute_stats_single` 逐维计算 DA 并记录不对齐维度列表。`max_misaligned_dims` 参数控制容忍度（默认 0 = 任何不对齐都标记）。
...
可通过扩展 `trajectory_da_filter` 添加互相关（cross-correlation）搜索最佳时移：先在 ±15 帧范围内搜索使 DA 最大化的 state-action 时间偏移量，在最优偏移下 DA 仍低于阈值时才判定该 episode 不合格。这一扩展可作为自定义算子的参数暴露出来。
