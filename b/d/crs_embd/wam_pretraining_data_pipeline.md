...
### 4.0 表征策略：原生表征 + 本体标签（已定）

异构混合数据下，**不把各数据集的动作统一成同一种物理量**（不做 IK/FK 把 EEF 反解成关节、也不反过来）。每个槽位保留各数据集的原生动作，靠三件事让模型消歧：

1. **语义槽位搬运**（§4.1）：把原始动作维 reorder 到统一语义槽位（左臂→左臂槽…）。只是搬运，不改物理量。
2. **per-embodiment 归一化**（§S3）：同一槽位里 A 是关节角、B 是 EEF delta，分布差异巨大 → 各自独立归一化后才落到同一数值范围。
3. **🔑 embodiment 条件**（§6 P0-b）：同槽位跨本体语义不同，**模型必须知道样本来自哪个本体**才能区分。实现为 per-embodiment learnable soft-prompt / embedding（参照 ActionCodec），注入 ActionDiT。

> **为什么选这条而不是「统一成关节/EEF」**：统一物理量需要每个本体的 URDF + IK/FK，开源混合数据集 URDF 经常缺失或不准，retarget 误差会污染标签。原生 + 标签把「消歧」这件事交给模型 + 归一化，工程量小、鲁棒，是 GR00T/OpenVLA 的成熟做法。
>
> **代价与缓解**：跨本体的同槽位权重共享收益变弱（语义不一致）。缓解靠 embodiment 条件 + per-emb 归一化；且本方案训的是**世界模型**，视频像素本身隐含本体信息，进一步降低风险。

**adapter 工作量（每个数据集一个，因此不统一表征而变轻）**：
- ✅ 必做：语义槽位映射（reorder）、频率重采样、声明 `embodiment_id` 与拓扑/mask。
- 🟡 建议：统一明显约定（夹爪 0/1 开合方向、底盘速度正方向）——能省样本；不做也能靠 per-emb 归一化 + 标签让模型自己学。
- ❌ 不做：IK/FK retarget、坐标系精确对齐、跨本体物理量统一。
